...
async function buildPersonSummary(groupId, personName) {
        ...
        const prompt = [
                ...
                '',
                '重要ルール：',
                '・タスク・役割として認識する条件：「@名前」の形式でLINEのメンション機能を使って明示的にメンションされている場合のみ',
                '・「@」マークなしで名前が出てくるだけの場合はタスクとして扱わない',
                '・例：「@りょーさん お願いします」→ タスクとして認識する',
                '・例：「りょーさんのところでも出てる」「りょーさんも確認して」→ タスクとして扱わない',
...
async function buildSummaryWithState(historyText, currentState) {
        ...
        if (hasState) {
                promptParts.push(
                        ...
                        '',
                        'ルール（必ず守ること）：',
                        '・タスク・担当として認識する条件：「@名前」の形式でLINEのメンション機能を使って明示的にメンションされている場合のみ',
                        '・「@」マークなしで名前が出てくるだけの場合はタスクとして扱わない',
