...
async function buildPersonSummary(groupId, personName) {
        ...
        const prompt = [
                ...
                '',
                '重要ルール：',
                '・タスク・役割として認識する条件：その人に対して明示的に作業・対応・確認を依頼・指示している発言がある場合のみ',
                '・単に名前が会話に出てくるだけ（例：「〇〇さんのところでも出てる」「〇〇さんも同じ現象」）はタスクとして扱わない',
                '・「〇〇さんお願い」「〇〇さんが担当」「〇〇さんやっておいて」など明示的な依頼・割り当てがある場合のみタスクに含める',
...
async function buildSummaryWithState(historyText, currentState) {
        ...
        if (hasState) {
                promptParts.push(
                        ...
                        '',
                        'ルール（必ず守ること）：',
                        '・タスク・担当として認識する条件：その人に対して明示的に作業・対応を依頼・指示している発言がある場合のみ',
                        '・単に名前が会話に登場するだけではタスク扱いしない',
